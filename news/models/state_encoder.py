...
class RLStateEncoder(nn.Module):
    ...
    @staticmethod
    def batch_data(x):
        numerical, node_feature, edge_tensor_1, edge_tensor_2, mask, stage = zip(*x)
        numerical = torch.stack(numerical)
        node_feature = torch.stack(node_feature)
        edge_tensor_1 = torch.stack(edge_tensor_1)
        edge_tensor_2 = torch.stack(edge_tensor_2)
        mask = torch.stack(mask)
        stage = torch.stack(stage)

        # Compatible with two observation layouts:
        # 1) [numerical, node_feature, edge_index_dis, edge_index_od, mask, stage]
        # 2) [numerical, node_feature, edge_feature, edge_index, mask, stage]
        if edge_tensor_1.dim() >= 3 and edge_tensor_1.shape[-1] == 2 and \
                edge_tensor_2.dim() >= 3 and edge_tensor_2.shape[-1] == 2:
            edge_index_dis = edge_tensor_1.to(torch.int64)
            edge_index_od = edge_tensor_2.to(torch.int64)
        elif edge_tensor_2.dim() >= 3 and edge_tensor_2.shape[-1] == 2:
            edge_index_dis = edge_tensor_2.to(torch.int64)
            edge_index_od = edge_tensor_2.to(torch.int64)
        elif edge_tensor_1.dim() >= 3 and edge_tensor_1.shape[-1] == 2:
            edge_index_dis = edge_tensor_1.to(torch.int64)
            edge_index_od = edge_tensor_1.to(torch.int64)
        else:
            raise ValueError(
                f'Unexpected GNN1 edge tensors: tensor1={tuple(edge_tensor_1.shape)}, '
                f'tensor2={tuple(edge_tensor_2.shape)}'
            )
